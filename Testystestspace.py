...
def play_game():
    ...
    quiz_entry = {'easy_quiz' :
    """Naruto graduates the Ninja Academy and joins Team ___1___, along side his rival ___2___, his crush ___3___ and his sensei ___4___.""",
            'intermediate_quiz':
    """The Akatsuki begin kidnapping the ___1___ to extract their ___2___ and revive the ___3___. With the ___3___ restored, the Akatsuki can cast the ___4___.""",
            'hard_quiz':
    ...
    def level_choice():
        ...
        for level in easyList:
            return quiz_entry['easy_quiz']
        if level in intermediateList:
            return quiz_entry['intermediate_quiz']
        elif level in hardList:
            return quiz_entry['hard_quiz']
        else:
            return quiz_entry['ultimate_quiz']
    ...
    def answer_question(index, quiz):
        ''' 
        ...
        while guesses > 0:
            guesses = input("What goes in ___%s___: " % str(index + 1))
            if check_answer(answer, guesses):
                print("CORRECT!! Way to go! \n")
                print('-' * 20)
                quiz = quiz.replace('___%s___' % str(index + 1), guesses)
                print(quiz)
                print('-' * 20 + '\n')
                break
            elif guesses > 1:
    ...
    def start_quiz():
        ...
        for index in range(len(quiz_answers[level])):
            quiz = answer_question(index, quiz)
            if not alive: break
